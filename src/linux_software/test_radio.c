...
void print_benchmark(volatile unsigned int *periph_base)
{
	...
	//count bytes sent then divide by clock cycles or period of clock cycle
	
	
	//unsigned int bytes_transferred = (stop_time-start_time)/8; // change obviously
    //float time_spent = stop_time-start_time; // change obviously
    //time_spent = (1/125E6) * time_spent; //125Mhz PL axi clock speed
	//float throughput=1/ time_spent /1000 /8 ; 
	

float bytes_transferred = (float) ((stop_time-start_time)*32)/8; // 32 bit wide write bus so each clock cycle transmits 32 bits
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 6 Daniel Wheeler's - Custom Peripheral Demonstration\n\r");
